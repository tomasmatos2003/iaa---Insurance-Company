...
def insert_data(
    ...
    vh_dict = request.dict()
    vh_dict_serializable = jsonable_encoder(vh_dict)

    vc_id = f"urn:uuid:{uuid4()}"
   
    subject_id = f"did:automobile:{current_user.username}"
    ...
    result = db["vehicles"].insert_one(signed_vc)
