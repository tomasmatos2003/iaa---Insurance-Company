  ...
  return (
    <AuthApi.Provider value={{ auth, setAuth }}>
      <TokenApi.Provider value={{ token, setToken }}>
        <Router>
          <div className="bg-red-50 min-h-screen p-4 font-sans">
            <nav className="mb-8">
...
function Register() {
  ...
  const handleSubmit = (evt) => {
    ...
    axios
      .post("http://127.0.0.1:8004/register", data)
      .then((response) => {
...
const Login = () => {
  ...
  const handleSubmit = async (evt) => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8004/login", formData, {
        headers: {
